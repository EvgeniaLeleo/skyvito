...
**_N.B._** При повторном запуске тестирования в тесте `./cypress/e2e/signupForm.ts' возникнет ошибка, поскольку пользователь с тем же e-mail не может быть зарегистрирован.

Поэтому при необходимости создать нового пользователя следует задать новое значение переменной `USER_EMAIL` в файле `./cypress/support/constants.ts'
